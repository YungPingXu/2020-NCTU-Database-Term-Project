...
load data local infile './dataset/animeListGenres.csv'
...
load data local infile './dataset/animeReviewsOrderByTime.csv'
...
load data local infile './dataset/AnimeList.csv'
...
from animelistgenres as al
left join (
	select workId, round(avg (
		overallRating + storyRating +
		animationRating + soundRating +
		characterRating + enjoymentRating) / 6) as rating
